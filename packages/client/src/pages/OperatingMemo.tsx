...
import { useRefreshIntelligence, useOperatorBrief } from '../hooks/useIntelligence';
    ...
    <div className="space-y-6">
        ...
        <div className="grid gap-8 p-6 lg:grid-cols-[1.5fr_0.9fr] lg:p-8">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-600 md:text-base">
              This memo is built from live price, recipe cost, and inventory variance signals. It is the fastest way to see whether the intelligent backend work is improving operating visibility.
            </p>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              >
                <RefreshCw size={16} className={refreshMutation.isPending ? 'animate-spin' : ''} />
                {refreshMutation.isPending ? 'Refreshing Intelligence...' : 'Refresh Intelligence'}
              </button>
              <a
      ...
      <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-3xl border border-border bg-bg-card p-5 shadow-sm">
          <SectionHeading
            ...
            subtitle="Highest-ranked operating items across all live intelligence packs."
          />
          <div className="mt-4 space-y-3">
            {topPriority.length > 0 ? topPriority.map((item) => (
              <PriorityCard key={`${item.signal_type}:${item.source_signal_id}`} item={item} />
            )) : (
              <EmptyState
      ...
      <section id="recommendations" className="rounded-3xl border border-border bg-bg-card p-5 shadow-sm">
        <SectionHeading
          ...
          subtitle="Durable operator actions synthesized from trusted signals, not just observations."
        />
            ...
            <div key={section.key} className="rounded-3xl border border-border bg-bg-card p-5 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                {section.items.length > 0 ? section.items.map((item) => (
                  <CompactMemoRow key={`${section.key}:${item.source_signal_id}`} item={item} />
                )) : (
                  <EmptyState
      ...
      <section className="rounded-3xl border border-border bg-bg-card p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-2">
          {brief.recent_signal_items.length > 0 ? brief.recent_signal_items.map((item) => (
            <CompactMemoRow key={`recent:${item.source_signal_id}`} item={item} />
          )) : (
            <div className="lg:col-span-2">
              <EmptyState
        ...
        <section className="rounded-3xl border border-amber-200 bg-amber-50/80 p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {needsReview.map((item) => (
              <CompactMemoRow key={`review:${item.source_signal_id}`} item={item} emphasizeFallback />
            ))}
          </div>
        </section>
      )}
...
function PriorityCard({ item }: { item: MemoItemPayload }) {
  return (
...
function CompactMemoRow({ item, emphasizeFallback = false }: { item: MemoItemPayload; emphasizeFallback?: boolean }) {
  return (
    <div className={`rounded-2xl border p-4 ${emphasizeFallback ? 'border-amber-200 bg-white/80' : 'border-slate-200 bg-slate-50/60'}`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
