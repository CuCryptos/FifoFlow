...
export function AllergyAssistant() {
  ...
  const [question, setQuestion] = useState('A guest has a guava allergy. Which products on this allergy chart are safe, which should be avoided, and which need kitchen confirmation?');
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-950">Allergy chart upload and product chat</h1>
            <p className="mt-3 text-sm leading-6 text-slate-600">
              Upload vendor or kitchen allergy charts, keep them scoped to the current venue, and ask grounded product questions. Answers come only from the uploaded chart products and cited chart evidence.
            </p>
          </div>
          <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            <div className="font-semibold">Current answer source</div>
            <div className="mt-1">Only uploaded allergy charts are used. No recipe or menu inference is allowed in this workflow.</div>
          </div>
        </div>
      </section>
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 rounded-3xl border border-dashed border-slate-300 bg-slate-50 p-5">
              ...
              <div className="mt-3 text-xs text-slate-500">
                Use venue-scoped charts whenever possible so product answers stay grounded in the correct chart library.
              </div>
              {files.length > 0 && (
        ...
        <div className="space-y-6">
          <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">The answer is grounded only in uploaded chart products and chart evidence in the current venue context.</p>
              </div>
            </div>
            ...
            <div className="mt-5 space-y-4">
              <textarea
                ...
                placeholder="A guest has a guava allergy. Which products on this chart are safe, which should be avoided, and which need kitchen confirmation?"
              />
              <div className="flex items-center justify-between gap-3">
                ...
                >
                  {askMutation.isPending ? 'Checking chart...' : 'Ask allergy chat'}
                </button>
              </div>
            </div>
          </div>
          ...
          {chatResponse ? (
            <div className="space-y-6">
              ...
              <ClassificationPanel
                title="Safe to serve"
                description="Only products with explicit support from the uploaded charts belong here."
              ...
              <ClassificationPanel
                title="Avoid"
                description="These products are directly implicated by the chart evidence for the asked allergy."
              ...
              <ClassificationPanel
                title="Ask kitchen"
                description="Ambiguous or may-contain products that need explicit kitchen confirmation."
                icon={<AlertTriangle className="h-5 w-5" />}
                tone="amber"
              ...
              <ClassificationPanel
                ...
                description="The uploaded charts did not support a grounded conclusion for these products."
            ...
            <div className="rounded-[28px] border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-600">
              Upload at least one allergy chart, then ask a chef-style question. The page will return grounded product guidance from the uploaded chart instead of a generic chatbot answer.
            </div>
          )}
        </div>
      </section>
    ...
    <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 sm:flex-row sm:items-start sm:justify-between">
      <div>
        ...
        <div className="mt-1 text-xs text-slate-500">
          {document.page_count} page{document.page_count === 1 ? '' : 's'} • {document.product_count} product{document.product_count === 1 ? '' : 's'} • {document.chunk_count} chunk{document.chunk_count === 1 ? '' : 's'} • {document.mime_type}
        </div>
    ...
    <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 space-y-3">
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <div key={`${title}-${item.product_id}`} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
              <div className="flex items-start justify-between gap-4">
                <div>
