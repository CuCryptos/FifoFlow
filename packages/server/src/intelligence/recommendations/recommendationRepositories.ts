...
import type { RecommendationSignalReadRepository } from './types.js';
import type { DerivedSignal, EvidenceReference } from '@fifoflow/shared';

export class SQLiteRecommendationSignalReadRepository implements RecommendationSignalReadRepository {
  constructor(private readonly db: Database.Database) {
    initializeIntelligenceDb(db);
  }

  async listSignalsForRecommendationWindow(
    window: { start: string; end: string },
    signalTypes?: Array<DerivedSignal['signal_type']>,
